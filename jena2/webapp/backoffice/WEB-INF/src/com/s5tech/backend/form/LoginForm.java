...
public class LoginForm extends ActionForm {
  ...
  public ActionErrors validate(ActionMapping mapping, HttpServletRequest req ){
    ...
    if(accessNumber == null || accessNumber.length() == 0) {
      String accessNumberLabel = resources.getMessage( "login.accessnumber" );
      ActionMessage newError =
        new ActionMessage("global.error.login.requiredfield", accessNumberLabel );
    ...
    if(pinNumber == null || pinNumber.length() == 0) {
      String pinNumberLabel = resources.getMessage( "login.pinnumber" );
      ActionMessage newError =
        new ActionMessage("global.error.login.requiredfield", pinNumberLabel );
      errors.add (ActionErrors.GLOBAL_MESSAGE, newError);
    }
    // Return the ActionMessages, in any.
    return errors;
  }
